...
| **版本迁移**（v3.0） | v1.x 旧格式项目自动检测 + 迁移指南（路径/格式/补建文件），不删除旧记忆 |
| **记忆缺口检测**（v3.1） | JSONL 时间戳 vs 代码文件 mtime，自动发现跨会话遗漏并补录 |
| **Step 5.5 合规自检**（v3.1） | 6 项文件逐项检查（DEV_MEMORY/JSONL/摘要/PROFILE/metadata/索引），杜绝"部分执行" |
...
将 SKILL.md 安装到你的 AI 工具中，使 AI 能自动加载完整的执行规范。

> **关键**：SKILL.md 顶部必须包含 YAML front matter（`---name/description---`），否则部分工具（如 TRAE）无法识别和注册 Skill。

根据你使用的 AI 工具，参考 [adapters/](adapters/) 目录中对应的适配文件进行安装。安装后**重启 AI 工具**，输入 `/` 检查 Skill 列表中是否出现 `dev-memory`。

### 2. 注入触发规则（必须）

根据你使用的 AI 工具，参考上方表格选择注入方式。TRAE 用户：在项目 Settings → Rules → Project Rules 中添加触发规则（详见 [adapters/TRAE.md](adapters/TRAE.md)）。

> **为什么需要两步？** Skill 安装提供执行规范（AI 知道"怎么保存"），触发规则注入确保每轮自动触发（AI 知道"何时保存"）。两者缺一不可。
...
如果你的项目已在使用 dev-memory v1.x（路径为 `docs/dev-memory/`），AI 会在开头检查时自动检测旧格式并提示迁移。也可手动迁移：

1. **路径迁移**：`mv docs/dev-memory/ dev-memory/`
2. **格式迁移**：在 DEV_MEMORY.md 顶部添加指纹行，每条记忆添加 `[引用:0]` 和价值标记
3. **补建文件**：复制 CONFIG.md + 创建 sessions/ + 创建 USER_PROFILE.md
4. **触发规则注入**：按当前 AI 工具参考 adapters/ 注入
5. **首次 Dream**：迁移后执行一次 Dream 整理

迁移不删除旧记忆，只追加格式标记。详见 SKILL.md "v1.x → v3.0 迁移指南"。
